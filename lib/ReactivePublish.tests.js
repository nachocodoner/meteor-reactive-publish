...
['STRING' /* 'MONGO' */].forEach((idGeneration) => {
  ...
  if (Meteor.isServer) {
    ...
    // Create a Map to store rerun counts outside the publish context
    ...
    // Method to get the current rerun count
    Meteor.methods({
    ...
    return [
      ...
      async function (next) {
        ...
        try {
          ...
          test.fail(`Failed to get initial rerun count: ${error.message}`);
          next();
        }
      },
      async function (next) {
      ...
      async function (next) {
        // Get the rerun count after stopping the subscription
        try {
          ...
          test.fail(`Failed to get rerun count after stop: ${error.message}`);
          next();
        }
      },
      async function (next) {
