  ...
  function basicSteps(publishName, test) {
    return [
      ...
      async function (next) {
        ...
        const filteredPostIds = postIds.filter((id) => id !== this.posts[0]);
        test.isTrue(arraysHaveSameItems(postIds, filteredPostIds));
    ...
    Meteor.publish(
      ...
      function (countId) {
        ...
        self.autorun(async (comp) => {
          ...
          // Publish the count
          self.added(`RerunCounts_${idGeneration}`, countId, {
            count: rerunCount,
          });
    ...
    Meteor.methods({
      ...
      [`getComputationState_${idGeneration}`](countId) {
        ...
        return {
          ...
          cursorCacheSize: computation._cursorCache
  ...
  function cleanupSteps(test) {
    return [
      ...
      async function (next) {
        ...
        try {
          await Meteor.callAsync(`insertCleanupTest_${idGeneration}`, {
            _id: 'test-id',
            value: 'test',
          });
          next();
      ...
      async function (next) {
        ...
        test.equal(
          doc?.value,
          'test',
          'Document should have the expected value'
        );
        ...
        try {
          ...
          test.isFalse(
            Object.keys(beforeCleanup.allCollectionNamesEmpty).filter(Boolean)
              .length > 0,
      ...
      async function (next) {
        ...
        try {
          ...
          test.isTrue(
            Object.keys(afterCleanup.allCollectionNamesEmpty).filter(Boolean)
              .length === 0,
    ...
    return [
      ...
      async function (next) {
        ...
        try {
          ...
          test.isTrue(
            this.initialRerunCount > 0,
            'Initial rerun count should be greater than 0'
          );
          ...
          test.isTrue(
            this.initialComputationState.exists,
            'Computation should exist'
          );
          test.isFalse(
            this.initialComputationState.stopped,
            'Computation should not be stopped initially'
          );
          test.isTrue(
            this.initialComputationState.cursorCacheSize === 1,
            'Cursor cache should have one entry'
          );
      ...
      async function (next) {
        ...
        try {
          ...
          test.isTrue(
            this.initialComputationState.cursorCacheSize === 1,
            'Cursor cache should have kept in one entry as same cursor'
          );
      ...
      async function (next) {
        ...
        try {
          ...
          test.isTrue(
            this.computationStateAfterStop.exists,
            'Computation should still exist after stopping'
          );
          test.isTrue(
            this.computationStateAfterStop.stopped,
            'Computation should be stopped'
          );
          test.equal(
            this.computationStateAfterStop.cursorCacheSize,
            0,
            'Cursor cache size should be 0 after stopping'
          );
  ...
  function userIdCheckSteps(test) {
    return [
      ...
      async function (next) {
        ...
        const UserIdCollection = new Mongo.Collection(
          `user-id-collection_${idGeneration}`
        );
