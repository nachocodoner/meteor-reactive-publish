...
['STRING' /* 'MONGO' */].forEach((idGeneration) => {
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - reactivity of single document operations (insert, update, remove)`,
    async function (test) {
  ...
  const Users = new Mongo.Collection(
    `Users_ReactiveMongoServer_tests_${idGeneration}`,
    { idGeneration }
  );
  const Posts = new Mongo.Collection(
    `Posts_ReactiveMongoServer_tests_${idGeneration}`,
    { idGeneration }
  );
  const Fields = new Mongo.Collection(
    `Fields_ReactiveMongoServer_tests_${idGeneration}`,
    { idGeneration }
  );

  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - findOneAsync with field projection and related collections`,
    async function (test) {
      await Test.find().forEachAsync(async (doc) => {
        await Test.removeAsync(doc._id);
      });
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - forEachAsync reactivity with document modifications`,
    async function (test) {
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - mapAsync reactivity with document transformations`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - fetchAsync reactivity with query options (sort, filter, limit, skip, projection)`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - nested autoruns with mongo operations`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - cursor cache clearing in all scenarios`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - hasPagination skips 'added' cb but preserves reactivity via 'addedBefore'`,
    async function (test) {
      // limit != null → hasPagination = true → shouldSkipAdded = true
      // The 'added' callback is omitted from cb; 'addedBefore' is used instead.
      // Reactivity must still fire on insert, update, and remove.
      const PagCol = new Mongo.Collection(
        `PagCol_${idGeneration}_${Random.id()}`,
        { idGeneration }
      );
      await PagCol.removeAsync({});

      let rerunCount = 0;
      let results = [];

      const computation = await AsyncTracker.autorun(async () => {
        rerunCount++;
        results = await PagCol.find(
          {},
          { limit: 5, sort: { value: 1 } }
        ).fetchAsync();
      });

      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 1, 'Initial run');
      test.equal(results.length, 0, 'No documents initially');

      // Insert — should rerun via 'addedBefore' (not 'added')
      const id1 = await PagCol.insertAsync({ value: 1 });
      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 2, 'Reruns on insert via addedBefore');
      test.equal(results.length, 1, 'Fetches inserted document');

      // Update — should rerun via 'changed'
      await PagCol.updateAsync(id1, { $set: { value: 2 } });
      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 3, 'Reruns on update');
      test.equal(results[0].value, 2, 'Reflects updated value');

      // Remove — should rerun via 'removed'
      await PagCol.removeAsync(id1);
      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 4, 'Reruns on remove');
      test.equal(results.length, 0, 'No documents after remove');
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - !isNamedCol skips 'added' cb but preserves reactivity via 'addedBefore'`,
    async function (test) {
      // Mongo.Collection(null) → collectionName = null → isNamedCol = false
      // shouldSkipAdded = true → 'added' cb is skipped; 'addedBefore' is used instead.
      // Reactivity must still fire on insert, update, and remove.
      ...
      const computation = await AsyncTracker.autorun(async () => {
        rerunCount++;
        results = await LocalCol.find({}).fetchAsync();
      });
      ...
      // Insert — should rerun via 'addedBefore' (not 'added')
      const id1 = await LocalCol.insertAsync({ value: 1 });
      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 2, 'Reruns on insert via addedBefore');
      test.equal(results.length, 1, 'Fetches inserted document');

      // Update — should rerun via 'changed'
      await LocalCol.updateAsync(id1, { $set: { value: 42 } });
      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 3, 'Reruns on update');
      test.equal(results[0].value, 42, 'Reflects updated value');

      // Remove — should rerun via 'removed'
      await LocalCol.removeAsync(id1);
      await Meteor._sleepForMs(100);
      test.equal(rerunCount, 4, 'Reruns on remove');
      test.equal(results.length, 0, 'No documents after remove');
  ...
  Tinytest.addAsync(
    `ReactiveMongoServer (${idGeneration}) - stopped computation doesn't rerun on collection changes`,
    async function (test) {
      // Create a test collection
      const StopTest = new Mongo.Collection(`StopTest_${Random.id()}`, {
        idGeneration,
      });

      // Clean up any existing documents
      await StopTest.find().forEachAsync(async (doc) => {
        await StopTest.removeAsync(doc._id);
      });
      ...
      // Create the autorun computation
      const trackerComputation = await AsyncTracker.autorun(async (computation) => {
        rerunCount++;
        ...
        // Fetch all documents
        fetchedResults = await StopTest.find({}, { sort: { value: 1 } }).fetchAsync();
      });
      ...
      // Verify initial state
      test.equal(rerunCount, 1, 'Computation should have run once initially');
      test.equal(fetchedResults.length, 0, 'No documents should be fetched initially');
      test.equal(computationIds.length, 1, 'Should be a single computation');

      // Insert test documents to trigger reruns
      const docIds = [];
      for (let i = 0; i < 3; i++) {
        const docId = await StopTest.insertAsync({
          value: i,
          name: `Item ${i}`,
        });
        docIds.push(docId);
      ...
      // Verify computation reran for each insert
      test.equal(rerunCount, 4, 'Computation should have rerun after each insert');
      test.equal(fetchedResults.length, 3, 'All documents should be fetched');
      test.equal(
        fetchedResults.map((doc) => doc.value),
        [0, 1, 2],
        'Documents should be sorted by value ascending'
      ...
      // Store the current rerun count
      const rerunCountBeforeStop = rerunCount;
      ...
      // Perform operations that would normally trigger reruns
      // Insert a new document
      await StopTest.insertAsync({
        value: 3,
        name: 'Item 3',
      });

      // Update an existing document
      await StopTest.updateAsync(docIds[0], { $set: { name: 'Updated Item 0' } });

      // Remove a document
      await StopTest.removeAsync(docIds[1]);

      // Wait to ensure any potential reruns would have happened
      await Meteor._sleepForMs(100);

      // Verify the rerun count hasn't changed after stopping
      test.equal(
        rerunCount,
        rerunCountBeforeStop,
        'Computation should not rerun after being stopped'
      );

      // Verify the fetched results haven't changed
      test.equal(
        fetchedResults.length,
        3,
        'Fetched results should remain unchanged after computation is stopped'
      );
      test.equal(
        fetchedResults.map((doc) => doc.value),
        [0, 1, 2],
