...
import { ReactiveVarAsync } from './async-reactive/ReactiveVarAsync';
...
['STRING' /*'MONGO'*/].forEach((idGeneration) => {
  ...
  if (Meteor.isClient) {
    ...
    Tinytest.addAsync(
      `PublishReactive multiple cursors (${idGeneration})`,
      (test, done) => {
  ...
  if (Meteor.isClient) {
    Tinytest.addAsync(
      `PublishReactive local collection (${idGeneration})`,
      (test, done) => {
  ...
  if (Meteor.isClient) {
    Tinytest.addAsync(
      `PublishReactive unblocked (${idGeneration})`,
      (test, done) => {
