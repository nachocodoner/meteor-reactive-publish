...
['STRING' /* 'MONGO' */].forEach((idGeneration) => {
  ...
  const Test = new Mongo.Collection(
    `Test_meteor_MongoReactive_tests_${idGeneration}`,
    { idGeneration }
  );

  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - reactivity of single document operations (insert, update, remove)`,
    async function (test) {
  ...
  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - findOneAsync with field projection and related collections`,
    async function (test) {
      await Test.find().forEachAsync(async (doc) => {
        await Test.removeAsync(doc._id);
      });
  ...
  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - forEachAsync reactivity with document modifications`,
    async function (test) {
  ...
  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - mapAsync reactivity with document transformations`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - fetchAsync reactivity with query options (sort, filter, limit, skip, projection)`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - nested autoruns with mongo operations`,
    async function (test) {
      // Create a test collection
  ...
  Tinytest.addAsync(
    `MongoReactiveServer (${idGeneration}) - cursor cache clearing in all scenarios`,
    async function (test) {
      // Create a test collection
