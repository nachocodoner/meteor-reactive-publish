...
if (isServer) {
  ...
  // Client-side implementation using Effect
  // We need a global reference to track the current computation
  let currentComputation = null;

  // Create a fiber-local reference for the current computation
  const CurrentCompRef = FiberRef.unsafeMake(null);

  // Runtime for synchronous operations
  let runtime = null;

  // Initialize the runtime
  Effect.runSync(
    Effect.gen(function* () {
      runtime = yield* Effect.runtime();
    })
  );
  ...
  asyncContext = {
    async run(store, fn) {
      ...
      // Enter computation only for the sync part of fn()
      currentComputation = store;
      let result;
      try {
        ...
        // Important: restore immediately after the sync enter
        if (currentComputation === store)
          currentComputation = previousComputation;
      }

      // If async, only set the computation during the continuation
    ...
    getStore() {
      // Try FiberRef first (kept for future Effect-based usage)
      if (runtime) {
        try {
          const fromFiber = Runtime.runSync(
            runtime,
            FiberRef.get(CurrentCompRef)
          );
