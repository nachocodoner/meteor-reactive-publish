...
import { AsyncTracker } from './ReactiveAsync/AsyncTracker';
    ...
    namesToWrap.forEach((callbackName) => {
      ...
      if (callback && typeof callback === 'function') {
        callbacks[callbackName] = (...args) => {
          publishContextStore.run(
            {
              publishContextStore: currentComputation,
            },
            () => {
              try {
                callback(...args);
              } catch (e) {
                // Skip errors within autorun callbacks as they are likely to be caused by
                // a race between the autorun and the publish function, and these are not breaking errors.
                if (['Could not find element', 'Removed nonexistent document'].some((msg) => e.message.includes(msg))) {
                  return;
                }
                throw e;
              }
